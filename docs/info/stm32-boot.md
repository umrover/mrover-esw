...
## The BOOT0 Pin
The BOOT0 pin only really matters for the first 4 clock cycles of the boot sequence after reset has been released. However, it decides where the chip will boot from: Main Flash, System Memory, or Embedded SRAM. (Maybe more or less options depending on the chip). By default, holding BOOT0 LOW will make the chip boot from the main flash, which is most likely where your code is. (When using CubeIDE, this is where it is typically flashed)
...
![STM32G4 boot mode selection table from the reference manual](boot-mode-selection.webp)\
