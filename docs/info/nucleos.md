...
![STM32 microcontroller family categories](img/MCU.webp)
...
![The range of available STM32 Nucleo boards](img/Nucleo.webp)
...
ST-Link's are also available as a separate piece of hardware, if a developer desires the ST-Link flashing/debugging experience with STM32 MCUs that aren't embedded into a Nucleo.
...
Using this function will also connect the CubeIDE to all the ST-Link's debugging functionalities. This way, the MCU is controllable through CubeIDE with standard debugging steps. This includes pausing at breakpoints, stepping forward, stepping in, stepping out, continuing, etc... Debugging a STM32 MCU with active interface peripherals is dangerous, however, as there is a good chance that the interface peripheral will enter faulty states because the MCU is not operating with normal execution flows.
...
![I2C interface block diagram for the STM32 F3 series](img/I2CHardware.webp)
