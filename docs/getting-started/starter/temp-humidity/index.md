...
## What Are Callbacks?
A callback is a function that is called automatically when an operation finishes (like I2C read and write operations). We don't need to make these function calls ourselves, the microcontroller instead does it for us. We will use I2C transmit and receive callbacks to handle communicating with the AHT20 sensor without having to block other code from finishing while we wait for transmits/receives to finish.

## Starter Code
The starter code lives in the `mrover-esw` repository, which you cloned during the
[LED Starter Project](../led/index.md#setting-up-the-repository). If you have not, do that first.

Open the project in STM32CubeMX.
...
Click `Run` -> `Debug As` -> `1 STM32 C/C++ Application`.

![The Run, Debug As, STM32 C/C++ Application menu path in CubeIDE](debug-as-menu.webp)

On the following screen, you can just press OK.

![The debug configuration prompt, which can be accepted as-is](debug-config-prompt.webp)

If your device is unable to detect the Nucleo connected to it, you may see the following screen.

![The error shown when CubeIDE cannot detect the connected Nucleo](nucleo-not-detected.webp)

Retry the previous steps and if that does not work, then either double check your connections, or seek a new Nucleo board.

In the future, you may be able to get away with just clicking `Run` -> `Debug`, or using the keyboard shortcut `F11`. This does not work the first time if you have never clicked on the Debug As button.

![The CubeIDE debug perspective, paused at a breakpoint](debug-perspective.webp)
