...
namespace mrover {
    ...
    HBridge::HBridge(TIM_HandleTypeDef* timer, std::uint32_t const channel, Pin const direction_pin)
    ...
    auto HBridge::write(Percent const output) const -> void {
        // Set direction pins/duty cycle
        set_direction_pins(output);
        set_duty_cycle(output, m_max_pwm);
    }

    auto HBridge::set_direction_pins(Percent const duty_cycle) const -> void {
    ...
    auto HBridge::set_duty_cycle(Percent duty_cycle, Percent const max_duty_cycle) const -> void {
    ...
    auto HBridge::change_max_pwm(Percent const max_pwm) -> void {
        m_max_pwm = max_pwm;
    }

    auto HBridge::change_inverted(bool const inverted) -> void {
