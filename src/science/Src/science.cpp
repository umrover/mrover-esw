...
namespace mrover {
    ...
    void event_loop() {
        while (true) {
            ...
            if (HAL_I2C_GetState(HI2C) == HAL_I2C_STATE_READY && restart_sensors) {
                mrover::science_board.restart_sensors();
                restart_sensors = false;
            }
            ...
            if (adc_free) {
                adc_free = false;
                science_board.poll_sensor(sensor_t::sensor_uv);
            }
        }
    }
    ...
    void init() {
        ...
        HAL_TIM_Base_Start_IT(SENSOR_RESTART_TIM);
    ...
    void update_i2c_queue() {
        ...
        if (current_sensor != sensor_t::sensor_co2)
...
extern "C" {
    ...
    void HAL_TIM_PeriodElapsedCallback (TIM_HandleTypeDef *htim) {
        if (htim == mrover::CO2_TX_TIM) {
            ...
            mrover::i2c_queue.push(mrover::sensor_t::sensor_co2);
    ...
    void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
        ...
        if (mrover::initialized)
            mrover::science_board.update_sensor(mrover::sensor_t::sensor_uv);
