...
void logger::test_factory(std::vector<std::unique_ptr<logger::Logger>> loggers) {
    ...
    assert(loggers[3]->ascii_log_file_path == "./log/canEpic.log");
}
...
int main() {
    ...
    logger::logger_factory(loggers, file_name);

    assert(loggers.size() == 4);

    logger::test_factory(loggers);
